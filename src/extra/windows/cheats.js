...
const godmode = new ui.Checkbox(window, "god mode", (val) => {
  data.set("configs/godmode", val);
});
godmode.element.checked = data.get("configs/godmode") || false;

const instantkill = new ui.Checkbox(window, "instant kill", (val) => {
  data.set("configs/onehit", val);
});
instantkill.element.checked = data.get("configs/onehit") || false;

const alwayscatch = new ui.Checkbox(window, "always catch", (val) => {
  data.set("configs/alwayscatch", val);
});
alwayscatch.element.checked = data.get("configs/alwayscatch") || false;

const freererolls = new ui.Checkbox(window, "free rerolls", (val) => {
  data.set("configs/freererolls", val);
});
freererolls.element.checked = data.get("configs/freererolls") || false;
